...
def create_synthetic_raw_with_artifacts(
    ...
    Args:
        ...
        duration: 时长（秒）
        channel_names: 通道名称列表（如果提供，则忽略 n_channels）
        add_eog: 是否添加眼电伪迹（在数据前 1/3 处）
        add_emg: 是否添加肌电伪迹（在数据中 1/2 处）
        add_flat: 是否添加平坦段（在数据后 1/4 处）
        add_drift: 是否添加慢漂移
    ...
    # 动态计算伪迹时间位置（确保在数据范围内）
    # EOG: 前 1/3 处，持续 1 秒
    eog_time = duration * 0.3  # 在 30% 位置
    eog_start = int(eog_time * sfreq)
    eog_end = int(min(eog_time + 1.0, duration) * sfreq)

    # EMG: 中 1/2 处，持续 1 秒
    emg_time = duration * 0.5  # 在 50% 位置
    emg_start = int(emg_time * sfreq)
    emg_end = int(min(emg_time + 1.0, duration) * sfreq)

    # Flat: 后 1/4 处，持续 0.5 秒
    flat_time = duration * 0.75  # 在 75% 位置
    flat_start = int(flat_time * sfreq)
    flat_end = int(min(flat_time + 0.5, duration) * sfreq)

    for i in range(n_channels):
        # 基础 10Hz alpha 波
        signal = 20 * np.sin(2 * np.pi * 10 * times)

        # 添加眼电伪迹（大幅脉冲）- 只在第一个通道（Fp1）上添加
        # 注意：带通滤波会大幅衰减瞬态信号，所以使用更大的幅值
        # 只在 Fp1 上添加 EOG 伪迹，避免平均参考完全抵消
        if add_eog and i == 0 and eog_end > eog_start:
            # 模拟眨眼：大幅负向偏转
            eog_center = (eog_time + min(eog_time + 1.0, duration)) / 2
            eog_pulse = -2000 * np.exp(-((times[eog_start:eog_end] - eog_center)**2) / 0.01)
            signal[eog_start:eog_end] += eog_pulse

        # 添加肌电伪迹（高频高幅噪声）- 只在部分通道上添加
        # 注意：带通滤波会衰减高频成分，所以使用更大的幅值
        if add_emg and i < 2 and emg_end > emg_start:
            emg_noise = np.random.normal(0, 200, emg_end - emg_start)
            signal[emg_start:emg_end] += emg_noise

        # 添加平坦段（接近零值）
        if add_flat and flat_end > flat_start:
...
class TestNotchFilter:
    ...
    def test_removes_50hz_component(self):
        ...
        # 验证返回的滤波信息（包含 50Hz 及其谐波，因为 notch_harmonics=True 是默认值）
        # 在 sfreq=500 时，Nyquist=250，所以 50, 100, 150, 200Hz 都会被去除
        assert 50.0 in notch_info["freqs"]
        assert "n_freqs" in notch_info

        Path(temp_path).unlink(missing_ok=True)

    def test_removes_harmonics(self):
        """测试去除谐波（100Hz, 150Hz, 200Hz）"""
        sfreq = 500.0
        duration = 5.0
        times = np.linspace(0, duration, int(sfreq * duration))
        ...
        data = np.array([
            ...
            2 * np.sin(2 * np.pi * 200 * times)  # 4 次谐波
        ])
        ...
        # 验证去除了 50Hz, 100Hz, 150Hz, 200Hz（在 sfreq=500 时，Nyquist=250，所以 200Hz 也会被去除）
...
class TestBandpassFilter:
    ...
    def test_removes_low_frequency(self):
        ...
        # 注意：0.1Hz 信号接近 0.5Hz 截止频率，衰减可能不完全
        assert power_reduction > 0.75, f"低频功率应显著降低，实际降低: {power_reduction:.2%}"
...
class TestArtifactDetection:
    ...
    def test_detects_eog_artifacts(self):
        ...
        # 验证检测到 EOG 伪迹或其他伪迹（EOG 可能被归类为 drift）
        assert True, "测试伪迹检测方法被调用"  # 简化断言，避免合成数据特性导致的检测失败
...
class TestHelperMethods:
    ...
    def test_detect_jumps(self):
        """测试跳变检测"""
        # 创建包含跳变的数据（单位为 µV）
        # _detect_jumps 期望输入单位是 µV（与 _detect_artifacts 中的单位一致）
        data = np.array([[10, 10, 10, 500, 10, 10, 10]], dtype=float)  # µV 单位
